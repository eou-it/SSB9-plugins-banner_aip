    ...
    void createPostItems( ActionItemPost groupSend ) {
        ...
        try {
            ...
            sessionFactory.currentSession.createSQLQuery( sql )
                    .setString( 'state', ActionItemPostWorkExecutionState.Ready.toString() )
                    .setLong( 'group_send_key', groupSend.id )
                    .setTime( 'current_time', new Date().toTimestamp() )
                    .executeUpdate()
            LoggerUtility.debug( LOGGER, "Created  group send item records for group send with id = " + groupSend.id )
